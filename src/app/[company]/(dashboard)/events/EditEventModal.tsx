...
function parseForm(fd: FormData) {
  return {
    ...
    recurrenceEndDate: fd.get("recurrenceEndDate") ? new Date(fd.get("recurrenceEndDate") as string) : null,
  };
}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.4)" }}>
          <div className="w-full max-w-lg rounded-[10px] p-6" style={{ backgroundColor: "#fff", border: "1px solid var(--border)", maxHeight: "90vh", overflowY: "auto" }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[16px] font-semibold" style={{ color: "var(--fg)" }}>Modifica evento</h2>
              <button onClick={() => setOpen(false)}><X className="w-4 h-4" style={{ color: "var(--fg-3)" }} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <EventFormFields defaults={{
                title: event.title,
                type: event.type,
                date: event.date,
                ...
                meetUrl: event.meetUrl ?? undefined,
