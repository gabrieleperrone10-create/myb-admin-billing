...
export function CreateFolderModal() {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="p-1 rounded" style={{ color: "var(--fg-3)" }} title="Nuova cartella">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.4)" }}>
          <div className="w-full max-w-sm rounded-[10px] p-6 space-y-4" style={{ backgroundColor: "#fff", border: "1px solid var(--border)" }}>
            <div className="flex items-center justify-between">
              <h2 className="text-[16px] font-semibold" style={{ color: "var(--fg)" }}>Nuova cartella</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <input name="name" required placeholder="es. Marketing" className="w-full px-3 py-2 rounded-[6px] text-[13px] outline-none" style={{ border: "1px solid var(--border)", color: "var(--fg)" }} />
              </div>
              <div>
                <label className="block text-[12px] font-medium mb-2" style={{ color: "var(--fg-2)" }}>Colore</label>
                <div className="flex gap-2 flex-wrap">
                  {COLORS.map(c => (
                    <button key={c} type="button" onClick={() => setColor(c)} className="w-7 h-7 rounded-full" style={{ backgroundColor: c, outline: color === c ? `2px solid ${c}` : "none", outlineOffset: 2, transform: color === c ? "scale(1.2)" : "scale(1)", transition: "transform 0.1s" }} />
                  ))}
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={loading} className="flex-1 py-2 rounded-[6px] text-[13px] font-medium" style={{ backgroundColor: color, color: "#fff" }}>{loading ? "..." : "Crea"}</button>
              </div>
            </form>
          </div>
        </div>
